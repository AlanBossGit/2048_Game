...
{
    //代码扩充倍率
    static float scriptNumber = 2f;
    //类名库文本：
    static string path = Application.dataPath  + "/Xyx/Editor/ExtentsClass/words.txt";
    //新生成类的位置：
    static string classPath = Application.dataPath  + "/Xyx/Scripts/SpecialScripts";
    //代码存在文件夹
    static string myClassPath= Application.dataPath+ "/Xyx/Scripts";
    //类名库：
    ...
    private static string CreateNameSpace()
	{
        ...
        string content = "namespace " + ca + na+ GetHuanHang(1) + "{";
        return content;
	}
    ...
    private static void CreateNewClass()
    {
        ...
        sb.Append(nameSpace);
        //3、创建类名头文本
        string headContent=CreateClassHead(true,className);
        ...
        {
            str += "\t\t\t index = UnityEngine.Random.Range(0,3);\n";
